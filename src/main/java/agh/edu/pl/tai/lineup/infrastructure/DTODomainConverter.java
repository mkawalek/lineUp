...
public class DTODomainConverter {
    ...
    public static Project fromProjectDTO(ProjectDTO projectDTO) {
        return new Project(
                ...
                projectDTO.getEnded(),
                projectDTO.getGitHubCollaborators()
        );
    }
    ...
    public static ProjectDTO toProjectDTO(Project project) {
        return new ProjectDTO(
                ...
                project.getTechnologies().getTechnologies().stream().map(Enum::name).collect(Collectors.toSet()),
                project.getParticipants().getProjectParticipants().stream().map(UserId::getValue).collect(Collectors.toSet()),
                project.getStatus().name(),
                project.getOwner().getValue(),
                project.getCreatedAt(),
                project.getGitHubCollaborators(),
                project.getEnded()
        );
    }
