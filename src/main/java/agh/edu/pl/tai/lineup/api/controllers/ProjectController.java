...
import agh.edu.pl.tai.lineup.infrastructure.github.GitHubApiRequester;
...
public class ProjectController {
    ...
    @Autowired
    public ProjectController(ProjectRepository projectRepository, UserRepository userRepository, GitHubApiRequester gitHubApiRequester) {
    ...
    public CompletableFuture<IdResponse> createProject(@RequestBody ProjectEntityRequest request, @LoggedUser AuthenticatedUser user) throws IOException {
        Project project = new Project(ProjectId.of(RandomIdGenerator.next()), request.getTitle(), request.getDescription(),
                request.getVersionControlUrl(), user.getUserId(), ApiDomainConverter.toProjectTechnologies(request.getTechnologies()), gitHubApiRequester.getCollaborators(request.getVersionControlUrl()));
